...
public class StudentForgotPasswordController implements Initializable {
    @FXML
    ...
    void SendRequest(ActionEvent event) {
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/classroom_management","root","");
            String sql = "SELECT * FROM student WHERE Student_ID=? AND Name=? AND Email=?";
            PreparedStatement statement = connect.prepareStatement(sql);
            ...
            if(result.next()){
                try {
                    ...
                    FXMLLoader loader = new FXMLLoader(getClass().getResource("StudentPasswordChange.fxml"));
                    root = loader.load();
                } catch (IOException ex) {
                    Logger.getLogger(LoginController.class.getName()).log(Level.SEVERE, null, ex);
                }
